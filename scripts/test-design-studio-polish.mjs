...
try {
  ...
  if (version !== "2.80.0") errors.push(`version ${version} != 2.80.0`);
  ...
  const layoutApi = await page.evaluate(() => ({
    ...
    profile: !!window.__DS_WALK_LAYOUT?.placementProfile,
    reapply: !!window.__DS_WALK_LAYOUT?.reapplyChamberSemantics,
    hit: !!window.__DS_WALK_LAYOUT_MODE?.placementHit
  }));
  ...
  if (!layoutApi.profile) errors.push("placementProfile missing");
  if (!layoutApi.reapply) errors.push("reapplyChamberSemantics missing");
  if (!layoutApi.hit) errors.push("placementHit missing on layout mode");
